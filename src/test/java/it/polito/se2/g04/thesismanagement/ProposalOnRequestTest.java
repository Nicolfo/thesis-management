...
public class ProposalOnRequestTest {
    ...
    public void setup() {
        ...
        ProposalOnRequest proposal1 = proposalOnRequestRepository.save(new ProposalOnRequest(
                1L, "titolo", "descrizione", teacher, student1, null, null, ProposalOnRequest.Status.PENDING
        ));
        ProposalOnRequest proposal2 = proposalOnRequestRepository.save(new ProposalOnRequest(
                2L, "titolo2", "descrizione2", teacher, student2, null, null, ProposalOnRequest.Status.PENDING
        ));
    ...
    public void teacherAcceptedTest() throws Exception {
        MvcResult res = mockMvc.perform(MockMvcRequestBuilders.put("/API/proposalOnRequest/updateStatus/teacherAccepted/{id}", 1L)
        ...
        mockMvc.perform(MockMvcRequestBuilders.put("/API/proposalOnRequest/updateStatus/teacherAccepted/"+new Random().nextLong(3, 100))
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    public void teacherRejectTest() throws Exception {
        MvcResult res = mockMvc.perform(MockMvcRequestBuilders.put("/API/proposalOnRequest/updateStatus/teacherRejected/{id}", 2L)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andReturn();
        String json = res.getResponse().getContentAsString();
        ...
        mockMvc.perform(MockMvcRequestBuilders.put("/API/proposalOnRequest/updateStatus/teacherRejected/"+new Random().nextLong(3, 100))
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isNotFound());
